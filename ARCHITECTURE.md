...
Never hard-code a link to a specific CMS slug from a static/shared component (Header, Footer, homepage sections) without checking its publish status first. An internal-link crawl during testing caught exactly this bug — the Header, Footer, and a homepage feature block all hard-linked to the still-draft `maritime-shipping-port-law` practice area, producing site-wide 404s (see `TESTING.md`). The fix at the time was a `cache()`-wrapped publish-status check shared between the layout and the homepage; that specific Maritime spotlight and its elevated nav link were later removed entirely per client direction (`SITE_STRATEGY.md` §1), so the helper (`src/lib/maritime.ts`) was deleted as dead code along with it. The general rule stands for any future component that would otherwise assume a known slug is live: `PracticeAreasGrid`, `IndustriesGrid`, `TeamPreview`, and `InsightsPreview` already only render items that come back from a `status: published` query, which is the safer default pattern to follow over a hard-coded link.
