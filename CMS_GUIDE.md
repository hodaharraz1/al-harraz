...
| `practice-areas` | Service pages (21 seeded: Civil Law, Litigation, Contracts, Real Estate, Debt Recovery, Inheritance, Family Law, Corporate/Commercial, Company Formation, Criminal Law, Administrative, Employment, Arbitration, Tax, Intellectual Property, Insurance, Consumer Protection, Banking & Finance, General Advisory, Maritime/Shipping/Port, Customs/Import-Export) | Yes | Seeded as **published** (generic, non-fabricated service-capability copy, per firm direction to execute and go live — see `CONTENT_REQUIRED.md`). Still worth a firm/legal review pass in `/admin`. `order` sets the business-priority display order (civil-first, maritime-last — `SITE_STRATEGY.md` §1); `featured` is now redundant for sorting but left in the schema. |
| `industries` | Sector pages, written from the client's business-problem angle | Yes | Seeded as 7 **published**. |
